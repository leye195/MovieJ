...
import { getKeyword, search } from "../reducers/search";
class SearchBar extends React.Component {
  ...
  render() {
    ...
    const result = results.map((item) => {
      return (
        <li className="search_item" key={item.id} id={item.id}>
          <a href={"/search?p=" + encodeURI(item.title)}>{item.title}</a>
        </li>
      );
    });
    return (
